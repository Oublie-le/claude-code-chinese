...
export function ClaudeInChromeOnboarding({ onDone }: Props): React.ReactNode {
  ...
  return (
    <Dialog
      title={t('chrome.onboarding.title')}
      onCancel={onDone}
      color="chromeYellow"
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          {t('chrome.onboarding.body')}
          {!isExtensionInstalled && (
            <>
              <Newline />
              <Newline />
              {t('chrome.onboarding.extension.required')}{' '}
        ...
        <Text dimColor>
          {t('chrome.onboarding.permissions')}
          {isExtensionInstalled && (
            <>
              {' '}
        ...
        <Text dimColor>
          {t('chrome.onboarding.moreinfo')}{' '}
          <Text bold color="chromeYellow">
            /chrome
          </Text>{' '}
          {t('chrome.onboarding.moreinfo.visit')} <Link url="https://code.claude.com/docs/en/chrome" />
        </Text>
      </Box>
    </Dialog>
  )
